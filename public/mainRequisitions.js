...
async function registerUser(event) {
	...
	if (!validatePassword(password)) return alert("Password requires at least 8 characters, with at least one upcase, a lowercase and a special character! Oss");
...
async function updateUser(usersId, currentName, currentAge, currentEmail) {
	...
	updNowButton.onclick = async function () {
		...
		if (newPassword && !validatePassword(newPassword)) return alert("Password requires at least 8 characters, with at least one upcase, a lowercase and a special character! Oss");
